...
help_text = (
    ...
    f"  {Fore.GREEN}/add{Style.RESET_ALL} - add new bot to list.\n\n"
    f"  {Fore.GREEN}/list{Style.RESET_ALL} - list all bots from file.\n\n"
    ...
    for line in conv:
        if 'USER:' in line:
            ...
            for bot in bots_list:
                nameb = f"{bot['name']} ({bot['expertise']})"
                if f'{nameb}:' in line:
...
def get_bot_response(uinput):
    ...
    bot_name="BOT"
    if bot:
        bot_name=f"{bot['name']} ({bot['expertise']})"
      #  print(f"Bot {bot_name} with expertise in '{bot['expertise']}' will respond.")
        bot_text=f"""
        your name is  {bot_name}, and your expertise is {bot['expertise']}, 
...
while True:
    try:   
        ...
        if "/add" in ui:
            name_bot = input(Fore.BLUE + Style.BRIGHT + 'What is the name of the bot? ' + Style.RESET_ALL).strip()
            expertise = input(Fore.BLUE + Style.BRIGHT + 'What is the bot’s expertise? (or press ENTER to leave empty) ' + Style.RESET_ALL).strip()
            style = input(Fore.BLUE + Style.BRIGHT + 'Any specific style you want it to behave? (or press ENTER to leave empty) ' + Style.RESET_ALL).strip()
        ...
        if "list" in ui:
            if bots_list:
                ...
                for index, bot in enumerate(bots_list, start=1):
                    ...
                    print(f"{index}. Name: {name} Expertise: {expertise} Style: {bot['style']}")
            else:
                print("No bots found.")
        ...
        if ("image" not in ui) and ("read" not in ui) and ("readfiles" not in ui) and ("add" not in ui) and ("list" not in ui):
            conversation_log.append(texttoadd)
            save_conversation(texttoadd)
            response = get_bot_response(ui)
            continue
